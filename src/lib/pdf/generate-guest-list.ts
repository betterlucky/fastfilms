...
function addPageNumber(doc: jsPDF) {
  ...
  for (let i = 1; i <= pages; i++) {
    doc.setPage(i)
    doc.text(`Page ${i} of ${pages}`, pageWidth / 2, doc.internal.pageSize.height - 10, { align: 'center' })
  }
}
...
export function generateGuestListPDF(data: GuestListData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    try {
      ...
      yPos += 8
      doc.text(`Date: ${formatDate(data.campaign.screeningDate)}`, MARGIN + 5, yPos)
      yPos += 8
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      summary.forEach((quantity, item) => {
        ...
        doc.text(`${quantity}`, MARGIN + CONTENT_WIDTH - 15, yPos, { align: 'right' })
        yPos += 7
      })
      yPos += 15
